...
void compileIfSt(void) {
  Instruction* falseJmp;
  Instruction* exitJmp;

  eat(KW_IF);
  compileCondition();
  falseJmp = genFJ(DC_VALUE);
  eat(KW_THEN);
  compileStatement();
  if (lookAhead->tokenType == KW_ELSE) {
    exitJmp = genJ(DC_VALUE);
    updateFJ(falseJmp, getCurrentCodeAddress());
    eat(KW_ELSE);
    compileStatement();
    updateJ(exitJmp, getCurrentCodeAddress());
  } else
    updateFJ(falseJmp, getCurrentCodeAddress());
}

void compileWhileSt(void) {
  CodeAddress startLabel;
  Instruction* falseJmp;

  eat(KW_WHILE);
  startLabel = getCurrentCodeAddress();
  compileCondition();
  falseJmp = genFJ(DC_VALUE);
  eat(KW_DO);
  compileStatement();
...
void compileForSt(void) {
  ...
  genCV();
  genLI();
  conditionLabel = getCurrentCodeAddress();
  type = compileExpression();
  checkTypeEquality(varType, type);
  genLE();
  falseJmp = genFJ(DC_VALUE);
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    ...
    elementSize = sizeOfType(arrayType->elementType);
    if (elementSize > 1) {
      genLC(elementSize);
      genML();
    }
