...
static int getScopeLevel(Scope* scope) {
  int level = 0;
...
void genVariableAddress(Object* var) {
  ...
  switch (var->kind) {
  case OBJ_VARIABLE:
    level = getScopeLevel(VARIABLE_SCOPE(var));
    genLA(level, VARIABLE_OFFSET(var));
    break;
  case OBJ_PARAMETER:
    level = getScopeLevel(PARAMETER_SCOPE(var));
    ...
    break;
  case OBJ_FUNCTION:
    level = getScopeLevel(FUNCTION_SCOPE(var));
    genLA(level, RETURN_VALUE_OFFSET);
    break;
  default:
    break;
  }
...
void genVariableValue(Object* var) {
  ...
  case OBJ_FUNCTION:
    level = getScopeLevel(FUNCTION_SCOPE(var));
    genLV(level, RETURN_VALUE_OFFSET);
    break;
  default:
    break;
  }
